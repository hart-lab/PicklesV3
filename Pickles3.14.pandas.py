...
unique_lineages = sorted( df.primary_disease.unique() )
...
for col in my_columns:
    essentiality_matrices[col] = df.drop( df.index.values[ np.isnan( df[col].values) ], axis=0 ).pivot( 
        index='Gene', columns='stripped_cell_line_name', values=col)
...
def corr_one_vs_Expr_all(gene, bf_mat, expr_mat):
    ...
    cov = np.dot(gene_essentiality_vector.values, expr_mat.T.values) - \
        n * (mu_x.values * mu_y.values)
    corrmatrix = cov / (s_x.values * s_y.values)
    outframe = pd.DataFrame(index=expr_mat.index.values, columns=[
...
def update_figure(dataset, gene1, gene2, selected_lineages, graph_type):
    ...
    if (graph_type == 'summary'):
        ...
        # calculate all corrs
        #
        ess_matrix = df.drop( df.index.values[ np.isnan( df[my_column].values) ], axis=0 )
        ...
        #my_correlates[gene1] = round( my_correlates[gene1].values, 3)
        ...
        if ('TKOv3' in my_column):
            ...
            #my_correlates[gene1] = my_correlates[gene1].values.round( decimals=3 )
            my_correlates.columns = ['PCC']
            my_correlates.index.name = gene1 
            top_corrs = my_correlates.nlargest(100, 'PCC')
            bot_corrs = my_correlates.nsmallest(100, 'PCC')
