...
df = pd.read_table('/Users/mcolic/Desktop/Data/master_table_Avana_Score_TKOv3_BF_Zscore_Expr_LOF_GOF_18959genes_1162cells_lineage_disease.txt', 
    dtype={ 
       'Gene':str,
       'Cell_Line':str,
...
my_correlates = corr_one_vs_all(
    gene1_default, essentiality_matrices[my_column_default])
...
app.layout = html.Div(className='container', children=[ 
    ...
    html.Div( className='PlotArea', children=[
       ...
       html.Div(id = 'essCors', children=[
            ...
            ], style={'display':'none', 'width':'100%','textAlign':'left', 'height':250})
...
@app.callback(
    [Output('topTable', 'data'), Output('topTable', 'columns'),
     Output('botTable', 'data'), Output('botTable', 'columns')],
    [Input('dataset', 'value'), Input('gene1', 'value')]
)
def populate_tables(dataset, gene1):
    my_column = dataset
    my_correlates = corr_one_vs_all(
        gene1, essentiality_matrices[my_column])
    ...
    top_data = top_corrs.reset_index().to_dict('records')
    top_cols = [{'name': gene1, 'id': gene1},
                {'name': 'PCC', 'id': 'PCC'}] 

    bot_data = bot_corrs.reset_index().to_dict('records')
    bot_cols = [{'name': gene1, 'id': gene1},
                {'name': 'PCC', 'id': 'PCC'}] 

    return [top_data, top_cols, bot_data, bot_cols]  



# @app.callback(
#     Output('topTable', 'active_cell'),
...
@app.callback(
    ...
    Input('botTable', 'active_cell')
    )    


#def update_figure(dataset, algo, gene1, gene2, graph_type):
def update_figure(dataset, gene1, gene2, graph_type, ess_top, ess_bot):
    ...
    if (graph_type == 'summary'):
        ...
        global top_corrs_ess
        global bot_corrs_ess
        ...
        if ((ess_top['column']==1) and (ess_bot['column']==1)):
            d2 = df[df.Gene == gene2][[my_column, 'stripped_cell_line_name']]
        ...
        elif (ess_top['column']==0) and (ess_top['row']>=ess_bot['row']):
            update_gene2 = top_corrs_ess.index.values[ess_top['row']] 
        ...
        elif (ess_bot['column']==0)  and (ess_top['row']<= ess_bot['row']):
            update_gene2 = bot_corrs_ess.index.values[ess_bot['row']] 
        ...
        d2 = df[df.Gene == gene2][[
            'Expr', 'stripped_cell_line_name', 'primary_disease']]
        d2.columns = ['Expr_' + gene2,
                      'stripped_cell_line_name', 'primary_disease']
        d = d1.merge(d2, on='stripped_cell_line_name')
        ...
        if ('TKOv3' in my_column):
            ...
            # calculate all corrs
            #
            my_correlates_ex = corr_one_vs_Expr_all(
                gene1, essentiality_matrices[my_column], expression_matrices)
            my_correlates_ex[gene1] = round(my_correlates_ex[gene1], 3)
            my_correlates_ex.columns = ['PCC']
            my_correlates_ex.index.name = gene1
            top_corrs_ex = my_correlates_ex.nlargest(10, 'PCC')
            bot_corrs_ex = my_correlates_ex.nsmallest(10, 'PCC')
            #
            display_area_context_text_content = html.Div(children=[
                ...
                html.Div(id='top_correlates', children=[
                    html.H3('Top positive correlates'),
                    dt.DataTable(
                        data=top_corrs_ex.reset_index().to_dict('records'),
                        columns=[{'name': i, 'id': i}
                                 for i in top_corrs_ex.reset_index().columns],
                        fixed_rows={'headers': True},
                        style_header={'fontSize': '14px', 'font-family': 'sans-serif',
                                      'fontWeight': 'bold', 'backgroundColor': '#e1e4eb'},
                        style_cell={'minWidth': 100, 'width': 100, 'maxWidth': 100,
                                    'fontSize': '12px', 'font-family': 'sans-serif'},
                        style_table={'height': 400, 'width': 220}
                    )
                ], style={'display': 'inline-block', 'padding-right': '5px'}),
                ...
                html.Div(id='bot_correlates', children=[
                    html.H3('Top negative correlates'),
                    dt.DataTable(
                        data=bot_corrs_ex.reset_index().to_dict('records'),
                        columns=[{'name': i, 'id': i}
                                 for i in bot_corrs_ex.reset_index().columns],
                        fixed_rows={'headers': True},
                        style_header={'fontSize': '14px', 'font-family': 'sans-serif',
                                      'fontWeight': 'bold', 'backgroundColor': '#e1e4eb'},
                        style_cell={'minWidth': 100, 'width': 100, 'maxWidth': 100,
    ...
    return fig, style0, info_area_context_text_content, style1,  display_area_context_text_content, style2, essStyle
