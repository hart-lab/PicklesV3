...
######################## v3.13 ######################## 
# click on top correlates in the co-essentiality and  #
# expression tabs updated the scatterplot             #
# (i.e. updates the comparison gene)                  #
####################################################### 
...
#
expression_matrices = []
expression_matrices = df.drop(df.index.values[np.isnan(df['Expr'].values)], axis=0).pivot(
...
def corr_one_vs_Expr_all(gene, bf_mat, expr_mat):
    ...
    other_vectors = expr_mat.drop(gene, axis=0)

    # Match the columns of bf_mat & expr_mat
    matched_columns = np.intersect1d(
        gene_vector.columns, other_vectors.columns)
    gene_vector = gene_vector[matched_columns]
    other_vectors = other_vectors[matched_columns]
    n = gene_vector.shape[1]
    ...
    s_x = gene_vector.std(1, ddof=n - 1)
    s_y = other_vectors.std(1, ddof=n - 1)
    cov = np.dot(gene_vector.values, other_vectors.T.values) - \
        n * (mu_x.values * mu_y.values)
    corrmatrix = cov / (s_x.values * s_y.values)
    outframe = pd.DataFrame(index=other_vectors.index.values, columns=[
                            gene], data=corrmatrix.T)
    return outframe



my_correlates = corr_one_vs_all(gene1_default, essentiality_matrices[my_column_default])
my_correlates[gene1_default] = round(my_correlates[gene1_default], 3)
my_correlates.columns = ['PCC']
my_correlates.index.name = gene1_default
top_corrs = pd.DataFrame(my_correlates.nlargest(10, 'PCC'))
bot_corrs = pd.DataFrame(my_correlates.nsmallest(10, 'PCC'))

top_corrs = top_corrs.reset_index().rename(columns = {gene1_default:'Positive correlates', 'PCC':'Positive PCC'})
bot_corrs = bot_corrs.reset_index().rename(columns = {gene1_default:'Negative correlates', 'PCC':'Negative PCC'})

corrs_df = pd.concat([top_corrs, bot_corrs], axis=1)



my_correlates_exp = corr_one_vs_Expr_all(gene1_default, essentiality_matrices[my_column_default], expression_matrices)
my_correlates_exp[gene1_default] = round(my_correlates_exp[gene1_default], 3)
my_correlates_exp.columns = ['PCC']
my_correlates_exp.index.name = gene1_default
top_corrs_exp = pd.DataFrame(my_correlates_exp.nlargest(10, 'PCC'))
bot_corrs_exp = pd.DataFrame(my_correlates_exp.nsmallest(10, 'PCC'))

top_corrs_exp = top_corrs_exp.reset_index().rename(columns = {gene1_default:'Positive correlates', 'PCC':'Positive PCC'})
bot_corrs_exp = bot_corrs_exp.reset_index().rename(columns = {gene1_default:'Negative correlates', 'PCC':'Negative PCC'})

corrs_exp_df = pd.concat([top_corrs_exp, bot_corrs_exp], axis=1)
...
app.layout = html.Div(className='container', children=[ 
    ...
    html.Div( className='PlotArea', children=[
        ...
        html.Div(id = 'essCors', children=[

            html.Div(id='top_correlates',  
                children = [
                html.H3('Top correlates', style = {'textAlign':'center'}),
                dt.DataTable(
                    id = 'topTable',
                    data=corrs_df.to_dict('records'),
                    columns= [{'name': i, 'id': i} for i in corrs_df.columns],
                    active_cell={'row':0, 'column':0},
                    fixed_rows={'headers': True},
                    style_header={'fontSize':'14px', 'font-family':'sans-serif','fontWeight': 'bold','backgroundColor': '#e1e4eb', 'textAlign':'center'},
                    style_cell={'minWidth': 150, 'width': 150, 'maxWidth': 150,'fontSize':'12px', 'font-family':'sans-serif', 'textAlign':'center'},
                    style_table={'height': 400, 'width':600},
                    editable = True
                    )
                ], 
                style={'display':'inline-block', 'padding-left':'5px'})], 
            style={'display':'none', 'width':'100%','textAlign':'center', 'height':250}),



        html.Div(id = 'expCors', children=[

                html.Div(id='top_correlates_exp',  
                children = [
                html.H3('Top correlates', style = {'textAlign':'center'}),
                dt.DataTable(
                    id = 'topTable_exp',
                    data=corrs_exp_df.to_dict('records'),
                    columns= [{'name': i, 'id': i} for i in corrs_exp_df.columns],
                    active_cell={'row':0, 'column':0},
                    fixed_rows={'headers': True},
                    style_header={'fontSize':'14px', 'font-family':'sans-serif','fontWeight': 'bold','backgroundColor': '#e1e4eb', 'textAlign':'center'},
                    style_cell={'minWidth': 150, 'width': 150, 'maxWidth': 150,'fontSize':'12px', 'font-family':'sans-serif', 'textAlign':'center'},
...
@app.callback(
    Output('topTable', 'data'), 
    [Input('dataset', 'value'), Input('gene1', 'value')]
)
def populate_ess_tables(dataset, gene1):
    my_column = dataset
    ...
    top_corrs = top_corrs.reset_index().rename(columns = {gene1:'Positive correlates', 'PCC':'Positive PCC'})
    bot_corrs = bot_corrs.reset_index().rename(columns = {gene1:'Negative correlates', 'PCC':'Negative PCC'})

    tops_table = pd.concat([top_corrs, bot_corrs], axis=1)
    data_ess = tops_table.to_dict('records')
   
    return data_ess


@app.callback(
    Output('topTable_exp', 'data'),
    [Input('dataset', 'value'), Input('gene1', 'value')]
)
def populate_tables(dataset, gene1):
    my_column = dataset
    my_correlates_exp = corr_one_vs_Expr_all(gene1, essentiality_matrices[my_column], expression_matrices)
    my_correlates_exp[gene1] = round(my_correlates_exp[gene1], 3)
    my_correlates_exp.columns = ['PCC']
    my_correlates_exp.index.name = gene1
    top_corrs_exp = pd.DataFrame(my_correlates_exp.nlargest(10, 'PCC'))
    bot_corrs_exp = pd.DataFrame(my_correlates_exp.nsmallest(10, 'PCC'))

    top_corrs_exp = top_corrs_exp.reset_index().rename(columns = {gene1:'Positive correlates', 'PCC':'Positive PCC'})
    bot_corrs_exp = bot_corrs_exp.reset_index().rename(columns = {gene1:'Negative correlates', 'PCC':'Negative PCC'})

    tops_table_exp = pd.concat([top_corrs_exp, bot_corrs_exp], axis=1)
    data_exp = tops_table_exp.to_dict('records')
    return data_exp
...
@app.callback(
    ...
    Output('essCors', 'style'),
    Output('expCors', 'style'),
    Input('dataset','value'),
    Input('gene1', 'value'),
    Input('gene2', 'value'),
    Input('graph_type', 'value'),
    Input('topTable', 'active_cell'),
    Input('topTable_exp', 'active_cell')
    )    


def update_figure(dataset, gene1, gene2, graph_type, ess_top, exp_top):
    ...
    essStyle = {'width':'100%','textAlign':'left', 'display': 'none'}
    expStyle = {'width':'100%','textAlign':'left', 'display': 'none'}
    ...
    if (graph_type == 'summary'):
        ...
        top_corrs_ess = my_correlates.nlargest(10, 'PCC')
        bot_corrs_ess = my_correlates.nsmallest(10, 'PCC')

        top_corrs_ess = top_corrs_ess.reset_index().rename(columns = {gene1:'Positive correlates', 'PCC':'Positive PCC'})
        bot_corrs_ess = bot_corrs_ess.reset_index().rename(columns = {gene1:'Negative correlates', 'PCC':'Negative PCC'})


        tops_table = pd.concat([top_corrs_ess, bot_corrs_ess], axis=1)
      
        if callback_context.triggered_id == 'gene2':  
            if (gene1 != gene2):
                d2 = df[ df.Gene==gene2][[my_column,'stripped_cell_line_name']]
                d = d1.merge(d2, on='stripped_cell_line_name', suffixes=("_" + gene1, "_" + gene2))
            else:
        ...
        elif (callback_context.triggered_id == 'topTable') and ((ess_top['column']==0) or (ess_top['column']==2)):
            update_gene2 = tops_table.iloc[ess_top['row'], ess_top['column']] 
            d2 = df[df.Gene == update_gene2][[my_column, 'stripped_cell_line_name']]
            d = d1.merge(d2, on='stripped_cell_line_name',
                                 suffixes=("_" + gene1, "_" + update_gene2))
            
            fig = px.scatter(d, x=my_column + '_' + gene1, 
                     y=my_column + '_' + update_gene2,
                     hover_name='stripped_cell_line_name',
                     trendline='ols',
                     trendline_scope='overall',
                     trendline_color_override='lightgray',
                     labels = {
                         my_column + '_' + gene1:  gene1 + ', ' + algo + ' (' + dataset + ')',
                         my_column + '_' + update_gene2:  update_gene2 + ', ' + algo + ' (' + dataset + ')',
                         'primary_disease' : 'Primary Disease'
                     },
                     category_orders = {
                        'primary_disease' : sorted( list( d.primary_disease.values ) )
        ...
        elif (callback_context.triggered_id == 'topTable') and ((ess_top['column']==1) or (ess_top['column']==3)):
            update_gene2 = tops_table.iloc[ess_top['row'], ess_top['column']-1] 
            d2 = df[df.Gene == update_gene2][[my_column, 'stripped_cell_line_name']]
            d = d1.merge(d2, on='stripped_cell_line_name',
                                 suffixes=("_" + gene1, "_" + update_gene2))
        ...
        essStyle = {'width':'100%','textAlign':'left', 'display': 'inline-block'}
        ...
        if ('TKOv3' in my_column):
            ...
            #
            ####
            # calculate and display top coexpression with query gene and comparison gene.
            ####
            #
            # calculate all corrs
            #
            ...
            my_correlates_exp.columns = ['PCC']
            my_correlates_exp.index.name = gene1
            top_corrs_exp = my_correlates_exp.nlargest(10, 'PCC')
            bot_corrs_exp = my_correlates_exp.nsmallest(10, 'PCC')
            ...
            elif (callback_context.triggered_id == 'topTable_exp') and ((exp_top['column']==0) or (exp_top['column']==2)):
                update_gene2 = tops_table_exp.iloc[exp_top['row'], exp_top['column']]
                d2 = df[df.Gene == update_gene2][['Expr', 'stripped_cell_line_name', 'primary_disease']]
                d2.columns = ['Expr_' + update_gene2,'stripped_cell_line_name', 'primary_disease']

                d = d1.merge(d2, on='stripped_cell_line_name')
                fig = px.scatter(d, x=my_column + '_' + gene1,
                                 y='Expr_' + update_gene2,
                                 hover_name='stripped_cell_line_name',
                                 trendline='ols',
                                 trendline_scope='overall',
                                 trendline_color_override='lightgray',
                                 labels={
                                     my_column + '_' + gene1:  gene1 + ', ' + algo + ' (' + dataset + ')',
                                     'Expr_' + update_gene2:  update_gene2 + ', log(TPM+1)',
                                     'primary_disease': 'Primary Disease'
                                 },
                                 category_orders={
                                     'primary_disease': sorted(d.primary_disease.values)
            ...
            elif (callback_context.triggered_id == 'topTable_exp') and ((exp_top['column']==1) or (exp_top['column']==3)):
                update_gene2 = tops_table_exp.iloc[exp_top['row'], exp_top['column']-1]
                d2 = df[df.Gene == update_gene2][['Expr', 'stripped_cell_line_name', 'primary_disease']]
                d2.columns = ['Expr_' + update_gene2,'stripped_cell_line_name', 'primary_disease']
            ...
            essStyle = {'width':'100%','textAlign':'left', 'display': 'none'}
            expStyle = {'width':'100%','textAlign':'left', 'display': 'inline-block'}
    ...
    return fig, style0, info_area_context_text_content, style1,  display_area_context_text_content, style2, essStyle, expStyle
